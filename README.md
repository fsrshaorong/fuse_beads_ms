...
- 拼豆：15 张图案，新访客从 50×50「莓果小物」开始，1159 颗、6 色、背景留空，使用 52×52 钉位的 Mini 板。旧 14 张图案和已有存档保留。数字选色，点击或连续拖动放豆；B 放豆、X 擦除，支持整笔撤销/重做。错色保留目标编号。
...
拼豆由 Blender 参数化生成，两套规格各有独立的未熨烫和成品网格。Mini 外径 2.61 mm、高 2.8 mm，使用 145 mm、52×52 钉板；Midi 外径 4.77 mm、高 5.07 mm，使用同宽的 29×29 钉板。界面和图案列表会显示相应规格，同一规格内不随图案大小缩放豆子。公开尺寸、孔径和钉距等估值，以及重建方法见 [拼豆模型说明](docs/BEAD_MODELS.md)。

新草莓的有效轮廓为 40×44，增加双色叶脉、弧形高光和 19 组奶油籽。旧草莓有效轮廓为 21×24；只移动相机或把旧格复制成更小的豆子，不能增加这些图案细节。新内容使用独立 ID 和色号，旧图案、颜色与存档签名保持原值。

![相同物理板宽与相机下的 Midi、Mini 原豆及 Mini 成品对照](docs/images/mini-density-comparison.png)

上图由独立组件自检生成，加载网页使用的真实 GLB、光照和材质；三列保持相同的物理板宽与相机，展示 Midi、Mini 原豆和 Mini 成品，不是完整工作室页面截图。

`.blend` 源文件为 `assets/models/bead-kit.blend` 与 `assets/models/mini-bead-kit.blend`；网页加载 `public/models/` 中对应的两个 GLB。房间家具仍为程序化美术样板；当前未做模型 LOD、音效、揭纸动画、真实热力/熔接模拟或成品自由旋转。桌面键鼠是当前验收目标，触屏未完成验收。
...
渲染自检会联网获取原站着色代码作为独立对照；已有缓存后可用 `npm run selfcheck:rendering -- --offline`。`selfcheck:beads` 保留主应用 29 格草莓的尺寸、拖动、熨烫、恢复和收藏回归。`selfcheck:mini` 用独立组件场景验证 Mini 图案、真实 GLB、着色和格位命中，不加载 `main` 或 HUD；它与主应用完整流程测试的范围不同。截图和报告用于检查实际效果，应用本身不需要该网络访问。
