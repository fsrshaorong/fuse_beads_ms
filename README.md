...
需要联机时，另开终端运行 `npm run start:server`（Node.js 24+），前端开发服务会把 `/socket.io` 代理到本机 `2567`。在网页创建小店后，点击「我的小店 → 新玩家打开测试」，即可用另一个身份测试同一房间；也可在独立浏览器窗口粘贴邀请链接加入。默认地址仅用于本机测试，尚未部署公网服务。
...
- 工作室：WASD 相对当前摄像机走动（W 朝镜头水平朝向，A/D 相对画面左右），人物朝实际行走方向转身；右键拖动观察，滚轮调整距离。走近工作台按 E 坐下。
