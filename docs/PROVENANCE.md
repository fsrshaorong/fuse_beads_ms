...
- 当前运行时只加载 `public/models/mini-bead-kit.glb`，源文件 `assets/models/mini-bead-kit.blend` 由 `scripts/blender/generate_mini_bead_kit.py` 生成。所有 22 张图案、散豆与收藏统一使用其中的 Mini 未熨烫和成品网格。两种状态均在 Blender 中原创建模，没有下载第三方豆子网格。
...
新增七款 50×50 图案由 `scripts/generate-detailed-patterns.mjs` 在最终格位上原创绘制，输出 `DetailedPatternData.ts`；没有下载第三方像素图，也没有最近邻放大旧图。名称、颗数、版本保留规则见 [图案美术说明](PATTERN_ART.md)。
