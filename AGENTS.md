...
- 渲染采用指定柔光玩具房的实际公式与配置。几何与笔触时钟冻结，正常镜头和工具动作独立运作。不要未经要求改回近似 PBR 混合、提亮、降饱和或新增后处理。
- TypeScript 使用严格类型、4 空格、Allman 花括号、分号、UTF-8 无 BOM；不引入 any，不批量改动无关文件。
- 变更后执行 npm test、npm run build、git diff --check。渲染改动执行 npm run selfcheck:rendering；交互或装配改动在本地服务上执行 npm run selfcheck。说明实际验证范围。
