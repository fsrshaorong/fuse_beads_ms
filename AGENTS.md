...
- 用户已授权添加独立联机后端。单机仍经 WorkshopApplication；联机经 RoomService 与 Core/Multiplayer 规则，提交后再广播确认。不要直接把单机命令广播给多人或用整板历史覆盖他人操作。后端使用 Node.js 24+ 与 SQLite；协议和接入范围见 docs/MULTIPLAYER.md，当前公网部署见 docs/DEPLOYMENT.md。改动联机规则或传输时执行 test:multiplayer；广播/持久化改动同时执行 selfcheck:multiplayer。
...
- Electron 客户端共用网页玩法；平台差异放在 src/Platform 与 desktop。主进程处理文件和 Steam 原生模块，渲染进程保持 sandbox/contextIsolation 开启、Node 集成关闭。客户端文件存档与浏览器存档隔离，测试必须指定临时数据目录；改动客户端入口、保存或窗口后执行 desktop:package 与 selfcheck:desktop。Steam 测试使用 480，不能把 SDK 初始化成功写成正式浮层、云存档或身份验证完成；详见 docs/DESKTOP.md。
- 用户已授权公网部署及接管旧端口，当前部署见 docs/DEPLOYMENT.md。服务器密码和私钥不写入项目；后续部署保持独立系统用户、SQLite 数据目录和发布目录分离。公网自检会产生测试房间，只在明确部署/验收工作中运行；普通测试继续使用临时数据库。
